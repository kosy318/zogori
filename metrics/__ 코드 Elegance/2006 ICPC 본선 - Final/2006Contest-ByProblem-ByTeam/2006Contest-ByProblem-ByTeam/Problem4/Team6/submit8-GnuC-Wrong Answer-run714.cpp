...
int main() {
	...
	while (T--) {
		...
		if (sta == eda && stb == edb) while(1);
		if (sta == edb && stb == eda) while(1);
		...
		else if (check[eda] == 1 && check[edb] != 0) {
			if (back[edb] != 1) while(1);
			sw = true;
		}
		else if (check[eda] != 0 && check[edb] == 1) {
			if (back[eda] != 1) while(1);
			sw = true;
		}
		if (sw) printf("YES\n"); else printf("NO\n");
	}
