...
int main()
{
	...
	while(tn--)
	{
		...
		bool flag1 = Process(want[0].first,want[1].first);
		bool flag2 = Process(want[0].second,want[1].second);
		...
		flag1 = Process(want[0].first,want[1].second);
		flag2 = Process(want[0].second,want[1].first);
