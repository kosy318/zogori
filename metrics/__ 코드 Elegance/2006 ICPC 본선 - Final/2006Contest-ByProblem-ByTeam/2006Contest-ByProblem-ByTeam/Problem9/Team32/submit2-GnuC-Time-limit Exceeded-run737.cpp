		...
		*/
		...
		for(int j=0;j<N-1;j++){
			...
			for(int k=0;k<N-1;k++){
				...
				int findpoint=inter(x[j],y[j],x[j+1],y[j+1],x[k],y[k],x[k+1],y[k+1],point);
				if(findpoint==1){
					possible[p_size]=point.xp;
					p_size++;
				//	cout<<"possible x "<<possible[p_size-1]<<endl;
				}				

			}
		}
		for(int x_i=0;x_i<p_size;x_i++){
		//	cout<<"for x_i = "<<x_i<<endl;
			double max_y=0;
			double this_y=0;
			for(int ff=0;ff<=N-2;ff++){
				height[ff]=gety(x[ff],y[ff],x[ff+1],y[ff+1],possible[x_i]);
		//		cout<<"heiight "<<ff<<" is "<<height[ff]<<endl;
				if(x[ff]<possible[x_i]&&x[ff+1]>possible[x_i]){
					this_y=y[ff]+(y[ff+1]-y[ff])*(possible[x_i]-x[ff])/(x[ff+1]-x[ff]);	
					//this_y=height[ff];
					/*	
					if(possible[x_i]==32){
						cout<<x[ff]<<" "<<y[ff]<<" "<<x[ff+1]<<" "<<y[ff+1]<<" "<<endl;
						cout<<"this "<<this_y<<endl;
					}
					*/
				}
			...
			if(max_y-this_y<ans)ans=max_y-this_y;
		}
