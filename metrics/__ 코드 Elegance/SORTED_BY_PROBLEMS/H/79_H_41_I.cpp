...
signed main()
{
    ...
    sort(xvt2.begin(), xvt2.end());

    for (auto & i : xvt2)
    {
        i = lower_bound(xvt.begin(), xvt.end(), i) - xvt.begin() + 1;
    }
    ...
    for (int i = 0; i<n;i++)
    {
        for (int j=i; j<n; j++)
        {
            ...
            for (int k = 0; k < n; k++)
            {
                ...
                //zcout << xl << " " << xr << " " << bottom << " " << point1[k].second << " " << q << endl;
                if (q < 0)
                    flag = true;
                ans = max(ans, q);
            }
