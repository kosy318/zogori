	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 0; i < allPoints.size(); i++) {
			for (int j = 0; j < allPoints.size(); j++) {
				...
				int strikeCount = strikeDP[eyi][exi] - strikeDP[syi - 1][exi] - strikeDP[eyi][sxi - 1];
				strikeCount += strikeDP[syi - 1][sxi - 1];
				int ballCount = ballDP[eyi][exi] - ballDP[syi - 1][exi] - ballDP[eyi][sxi - 1];
				ballCount += ballDP[syi - 1][sxi - 1];

				int result = C1 * strikeCount - C2 * ballCount;
