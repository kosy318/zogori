...
int main(){
    ...
    for(int i=0;i<p.size();i++){
        for(int j=i;j<p.size();j++){
            ...
            if(i_x<=j_x && i_y<=j_y) {
                s = sum[j_x][j_y].first - sum[i_x - 1][j_y].first - sum[j_x][i_y - 1].first +
                    sum[i_x - 1][i_y - 1].first;
                b = sum[j_x][j_y].second - sum[i_x - 1][j_y].second - sum[j_x][i_y - 1].second +
                    sum[i_x - 1][i_y - 1].second;
            }
            else{
                swap(i_y,j_y);
                s = sum[j_x][j_y].first - sum[i_x - 1][j_y].first - sum[j_x][i_y - 1].first +
