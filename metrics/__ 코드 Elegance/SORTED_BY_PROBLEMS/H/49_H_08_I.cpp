...
int main()
{
    ...
    for(ll i=0; i<b.size(); i++)
        ...
        {
            ll maxx = max(b[i].first, b[j].first);
            ll minx = min(b[i].first, b[j].first);
            ll maxy = max(b[i].second, b[j].second);
            ll miny = min(b[i].second, b[j].second);

            ans = max(ans, sol(minx, miny, maxx, maxy));
        }
