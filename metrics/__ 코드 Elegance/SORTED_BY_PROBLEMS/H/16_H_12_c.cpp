...
struct node1 {
    int min = INT_MAX / 2, max = INT_MIN / 2, val = 0;
    node1(int min = INT_MAX / 3, int max = INT_MIN / 3, int val = 0) : min(min), max(max), val(val) {}
};

std::vector<int> Y;
int lazy[2048 * 2 + 1], a[2048];
node1 tree[2048 * 2 + 1];
info input[2048];
...
node1 query(int node, int start, int end, int i, int j) {
    lazy_propagation(node, start, end);
    if(i > end || j < start) {
        return node1(INT_MAX / 3, INT_MIN / 3, 0);
    }
    if(i <= start && end <= j)
        return tree[node];
...
int main(void) {
    ...
    for(int i = 1; i < n; i++) {
        ...
        init(1, 0, Y.size());
        r = std::max(r, query(1, 0, Y.size(), 0, Y.size()).val);

        for(int j = i + 2; j <= n; j++) {
            update(1, 0, Y.size(), input[j - i - 1].y, Y.size(), -1 * input[j - i - 1].val);
            update(1, 0, Y.size(), input[j].y, Y.size(), input[j].val);
            r = std::max(r, query(1, 0, Y.size(), 0, Y.size()).val);
        }
    }
