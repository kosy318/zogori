...
ll x[100005], clr[100005], gmin[100005], gTN[100005];
vector<ll> g[100005], d[100005];
vector<vector<ll>> dtree;

ll range_min(vector<ll> &tree, ll ql, ll qr, ll idx, ll l, ll r){
...
int main()
{
    ...
    gTN[0] = 1; dtree.push_back(vector<ll>(2, inf));
    update(dtree[0], gTN[0] + 1 - 1, 0);
    for(int gi = 1; gi <= G; gi++){
        for(gTN[gi] = 1; gTN[gi] < g[gi].size(); gTN[gi] <<= 1);
        dtree.push_back(vector<ll>(2 * gTN[gi], inf));

        int pN = g[gi - 1].size(), l = 0, r = pN - 1;
        int curx = g[gi][0], S = g[gi][0], E = g[gi - 1][(ll)g[gi - 1].size() - 1];
