...
int main(void) {
	...
	for(int i=1;i<=n;i++){
		if(head[i] == 1) seg.update(i, 1);
			...
			int ph = head[h-1];
			int px1 = lower_bound(x+1, x+1+n, 2*x[h-1]-x[i]) - x;
			px1 = max(ph, px1);
			int px2 = upper_bound(x+1, x+1+n, 2*x[h]-x[i]) - x - 1;
			
			px2 = min(px2, h-1);
			//debug(px1, px2);
			if(px1 <= px2) ret = min(ret, seg.get_min(px1, px2));
			seg.update(i, ret+1);
			//debug(i, ret);
		}
	}
	cout << seg.get_min(head[n], n);
}	
