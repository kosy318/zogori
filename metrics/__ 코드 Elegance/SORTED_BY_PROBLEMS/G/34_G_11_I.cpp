...
int main() {
    ...
    for(int i=0; i<cont.size()-1; i++){
        for(int j=0; j<cont[i].size(); j++){
            int p = cont[i][j];
            int d = p - cont[i][0];
            int L = lower_bound(cont[i+1].begin(), cont[i+1].end(), p+A[i]-d-M[i]) - cont[i+1].begin();
            int R = upper_bound(cont[i+1].begin(), cont[i+1].end(), p+A[i]-d+M[i]) - cont[i+1].begin();
            //cout<<i<<j<<' '<<A[i]-d<<M[i]<<' '<<L<<R<<endl;
