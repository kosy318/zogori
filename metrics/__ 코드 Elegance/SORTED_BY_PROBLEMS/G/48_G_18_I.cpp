...
int main()
{
    ...
    for(int gi = 1; gi <= G; gi++){
        ...
        int pN = g[gi - 1].size(), l = pN - 1, r = pN - 1;
        int curx = g[gi][0], S = g[gi][0], E = g[gi - 1][(int)g[gi - 1].size() - 1];
        while(r >= 0 && (curx - S > S - g[gi - 1][r] || curx - E < E - g[gi - 1][r])) r--;
        l = r;
        while(l >= 0 && (curx - S <= S - g[gi - 1][l] || curx - E >= E - g[gi - 1][l])) l--;
//        printf("%d\n", dtree[gi - 1].size());
//        printf("%d\n", range_min(dtree[gi - 1], 0, gTN[gi - 1] - 1, 1, 0, gTN[gi - 1] - 1));
        d[gi].push_back(range_min(dtree[gi - 1], l + 1, r, 1, 0, gTN[gi - 1] - 1) + 1);
