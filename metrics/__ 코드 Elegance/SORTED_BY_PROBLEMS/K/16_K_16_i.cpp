...
int main(void) {
    ...
    if(k == 1) {
        ...
        for(int i = 1; i <= n; i++)
            v.push_back(input[i]);
        printf("%.6lf\n", (getCost(v) * 10000000 / (n * n)) / 10000000.0);
    }
    else if(n == 1 || (k == 2 && n == 2)) {
        printf("0.000000\n");
    }
        ...
        for(int i = 1; i <= n; i++)
            v.push_back(input[i]);
        long long r = getCost(v) * 10000000LL / (n * n);
        ...
        for(int i = 1; i <= n; i++) {
            for(int j = i; j <= n; j++) {
                ...
                if(v1.size() == 0) {
                    r = std::min(r, (long long)getCost(v2) * 10000000LL / (long long)(v2.size() * v2.size()));
                }
                else if(v2.size() == 0) {
                    r = std::min(r, (long long)getCost(v1) * 10000000LL / (long long)(v1.size() * v1.size()));
                }
                else
                    r = std::min(r, (long long)getCost(v1) * 10000000LL / (long long)(v1.size() * v1.size()) + (long long)getCost(v2) * 10000000LL / (long long)(v2.size() * v2.size()));
            }
        }
        printf("%.6lf\n", r / 10000000.0);
    }
