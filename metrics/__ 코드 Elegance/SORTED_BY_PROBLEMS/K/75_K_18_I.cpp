    ...
    for(int i=0;i<n;i++){
        for(int j=i+1;j<n;j++){
            for(int l=j+1;l<n;l++) {
                ...
                int uy=z1*x2-z2*x1;
                int uz=x1*y2-x2*y1;
                long long int cmp=(long long int)ux*p[i].x+uy*p[i].y+uz*p[i].z;

                int sx1=0, sy1=0, sz1=0, c1=0;
                int sx2=0, sy2=0, sz2=0, c2=0;
                for(int k=0;k<n;k++){
                    long long int q = (long long int)ux*p[k].x+uy*p[k].y+uz*p[k].z;
                ...
                for(int t=0; t<8; t++) {
                    ...
                    if(_c1!=0 && _c2!=0){
                        ans=min((double) -(_sx1*_sx1+_sy1*_sy1+_sz1*_sz1)/(_c1)-(_sx2*_sx2+_sy2*_sy2+_sz2*_sz2)/(_c2)+sxx+syy+szz, ans);
                    }
                }
            }
        }
