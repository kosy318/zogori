...
double get(vector<bool>& state){
    ...
    //cout << x1sum << ' ' << x2sum << '\n';
    double ret = -1.0*(x1sum*x1sum+y1sum*y1sum+z1sum*z1sum)/n1 + x1sqsum+y1sqsum+z1sqsum;
    ret += -1.0*(x2sum*x2sum+y2sum*y2sum+z2sum*z2sum)/n2 + x2sqsum+y2sqsum+z2sqsum;
    //cout << ret << '\n';
    return ret;
}
int main(void){
    ...
    if(k==1){
        ...
        //cout << xsum << '\n';
        double ans = -1.0*(xsum*xsum)/n + -1.0*(ysum*ysum)/n + -1.0*(zsum*zsum)/n + xsqsum+ysqsum+zsqsum;
        cout << ans;
        return 0;
    }
