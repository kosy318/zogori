...
ll m,n,price[105], ans[16005], rans;
llp sep;
...
int main(){
    ...
    for(int i =1;i<=n;i++){
        ...
        pq.push({e, {s, (e - s) * price[t]}});
    }
    for(int sec = 1;sec <= 16000;sec++){
        if(pq.empty()) break;
        ans[sec] = ans[sec - 1];
        while(!pq.empty() && pq.top().first == sec){
            int e = pq.top().first, s = pq.top().second.first, cost = pq.top().second.second;
            ans[sec] = max(ans[sec], ans[s] + cost);
            rans = max(rans,ans[sec]);
            pq.pop();
        }
    }
    printf("%lld\n",rans);
