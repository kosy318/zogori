...
int val[10001];
int finish[10001];
pair<pair<int,int>,int> pii[10001];
...
int main(){
    ...
    for(int i = 1; i<=check_cnt; i++){
        scanf("%d%d%d",&pii[i].first.second,&pii[i].first.first,&pii[i].second);
    }

    sort(pii+1, pii+1+check_cnt);

    for(int i = 1; i<=check_cnt; i++){
        int ftime, stime, val_num;
        ftime = pii[i].first.second;
        stime = pii[i].first.first;
        val_num = pii[i].second;
        int check_max = 0;
        for(int k = i-1; k>=1; k--){
